...
export const useArenaStore = createPersistStore(
  ...
  (set, get) => ({
    ...
    createDuel(
      ...
      const currentDuels = get().duels;
      set({
        duels: [...currentDuels, duel],
      });
    ...
    submitVote(arenaId: string, blindLabel: string, vote: VoteResult): boolean {
      const state = get();
      ...
      const updatedDuels = [...state.duels];
    ...
    submitTieVote(arenaId: string): boolean {
      ...
      const statsUpdate = getUpdatedStatsFromDuel(
        completedDuel,
        state.pairwise,
        state.eloRecords,
      );
      ...
      set({
        duels: updatedDuels,
        pairwise: statsUpdate.pairwise,
        eloRecords: statsUpdate.eloRecords,
      });
    ...
    getModelElo(model: string, providerName: string): ModelEloRecord {
      ...
      return (
        get().eloRecords.find(
          (e) => getModelKey(e.model, e.providerName) === key,
        ) ?? {
          model,
          providerName,
          elo: DEFAULT_ELO,
...
function getUpdatedStatsFromDuel(
  duel: ArenaDuelRecord,
  currentPairwise: PairwiseRecord[],
  currentEloRecords: ModelEloRecord[],
): { pairwise: PairwiseRecord[]; eloRecords: ModelEloRecord[] } {
  const pairwiseMap = new Map(currentPairwise.map((p) => [p.id, p]));
  const eloMap = new Map(
    currentEloRecords.map((e) => [getModelKey(e.model, e.providerName), e]),
  );
  ...
  if (betterLabels.length === 1 && worseLabels.length === 1) {
    ...
    if (!pairwise) {
      ...
      pairwise = {
        ...
        modelAProvider: isAReversed ? loser.providerName : winner.providerName,
        modelB: isAReversed ? winner.model : loser.model,
        modelBProvider: isAReversed ? winner.providerName : loser.providerName,
        winsA: 0,
        winsB: 0,
        ties: 0,
      };
  ...
  return {
    pairwise: Array.from(pairwiseMap.values()),
    eloRecords: Array.from(eloMap.values()),
  };
}
